...
const PresetterCommandGenerator: React.FC = () => {
    ...
    switch (options.command) {
      case 'bootstrap':
        return [
          ...
          'presetter bootstrap --packages "@scope/*"',
        ];
      case 'run':
        return [
          'presetter run build',
    ...
    <div
      ...
      <div
        ...
        {options.command === 'run' && (
          <div>
            ...
            <input
              ...
              onChange={(e) =>
                setOptions((prev) => ({ ...prev, script: e.target.value }))
              }
            />
          </div>
        )}
        ...
        {(options.command === 'run-s' || options.command === 'run-p') && (
          <div>
            ...
            <input
              placeholder={
                options.command === 'run-s' ? 'clean build' : 'lint typecheck'
              }
              style={{
                width: '100%',
                padding: '8px',
              ...
              onChange={(e) =>
                setOptions((prev) => ({ ...prev, pattern: e.target.value }))
              }
            />
          </div>
        )}
        ...
        {options.command === 'run' && (
          <div>
            ...
            <input
              ...
              value={options.args}
              onChange={(e) =>
                setOptions((prev) => ({ ...prev, args: e.target.value }))
              }
            />
          </div>
        )}
      ...
      <div
        ...
        <ul style={{ margin: '8px 0', paddingLeft: '20px' }}>
          <li>
            <strong>bootstrap</strong>: Set up initial configuration files
          </li>
          <li>
            <strong>run</strong>: Execute a single script with optional
            arguments
          </li>
          <li>
            <strong>run-s</strong>: Run scripts one after another (sequential)
          </li>
          <li>
            <strong>run-p</strong>: Run scripts at the same time (parallel)
          </li>
        </ul>
      </div>
    </div>
...
export default PresetterCommandGenerator;
