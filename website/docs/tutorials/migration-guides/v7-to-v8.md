...
- **Standalone Storybook preset for web projects (v8.2+)**
...
- **TailwindCSS import resolution** fixes for web projects (v8.0.3+)
...
Don't worry - npm will automatically install them as peer dependencies when you install the Presetter packages.
...
If you're using `presetter-preset-web`, you now get TailwindCSS improvements. Add the Storybook preset separately when you need component development:

```bash
# Install the Storybook preset for Storybook scripts
npm install --save-dev presetter-preset-storybook
```
