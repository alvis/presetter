...
Bootstrap reads an existing `presetter.config.ts`; create that file before running the command if your project does not already have one.
...
export default preset('react-project', {
  extends: [presetReact],
  // Custom variables
  variables: {
...
Replace direct tool commands with delegates to Presetter's script runner:
...
```bash
# Review package changes you made during migration
