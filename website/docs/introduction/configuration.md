...
variables: {
  ...
  name: (context) => context.packageJson.name,
...
export default preset('adaptive', (context) => {
  ...
  const isLibrary = context.packageJson.main !== undefined;
...
assets: {
  ...
  '.storybook/main.ts': (current, { context }) => {
    if (!context.packageJson.devDependencies?.['@storybook/react']) {
...
Use debug logging while bootstrapping or running a preset script:

```bash
# Print bootstrap diagnostics
DEBUG=presetter:* npx presetter bootstrap

# Print script-resolution diagnostics
DEBUG=presetter:* npx presetter run build
```
