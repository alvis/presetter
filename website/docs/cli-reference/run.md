...
## Preset Binary Resolution (v8.3+)

When Presetter runs a task, it now adds each preset's `root` bin path to `PATH`.
That means scripts like `eslint`, `vitest`, or `storybook` resolve even when their
binaries come from preset packages instead of your project's direct dependencies.

If you need to force a local binary, install it directly or call it with an explicit path.
